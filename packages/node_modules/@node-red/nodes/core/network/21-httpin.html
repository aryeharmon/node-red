...
<script type="text/x-red" data-template-name="http in">
    ...
    <div class="form-row">
        <label for="node-input-layout"><i class="fa fa-wpforms"></i> Layout</label>
        <select type="text" id="node-input-layout" style="width:70%;"></select>
    </div>
    ...
    <div class="form-row-http-in-security hide">
        <div class="form-row">
            <label for="node-input-api_category"><i class="fa fa-wpforms"></i> API Category</label>
            
            <select id="node-input-api_category">
            </select>
        </div>
        <div class="form-row">
            <label for="node-input-description"><i class="fa fa-wpforms"></i> API Description</label>
            
            <select id="node-input-description">
            </select>
        </div>
        <div class="form-row">
            <label for="node-input-api_enabled"><i class="fa fa-wpforms"></i> API Enabled</label>
            <input type="checkbox" id="node-input-api_enabled" style="display: inline-block; width: auto; vertical-align: top;">
        </div>
        <div class="form-row">
        ...
        <div class="form-row">
            <label for="node-input-required_fields"><i class="icon-tag"></i> Required Fields</label>
            <select type="text" id="node-input-required_fields" multiple="multiple"></select>
        </div>
        <div class="form-row">
            <label for="node-input-available_fields"><i class="icon-tag"></i> Required Fields</label>
            <select type="text" id="node-input-available_fields" multiple="multiple"></select>
        </div>
        <div class="form-row">
            <label for="node-input-security_enabled"><i class="fa fa-wpforms"></i> Security Enabled</label>
            <input type="checkbox" id="node-input-security_enabled" style="display: inline-block; width: auto; vertical-align: top;">
        </div>
	<div class="form-row">
            <label for="node-input-security_type"><i class="fa fa-wpforms"></i> Security Type</label>
            <select type="text" id="node-input-security_type" style="width:70%;">
    ...
    <div class="form-row form-row-http-in-verify_captcha">
        <label>&nbsp;</label>
        <input type="checkbox" id="node-input-verify_captcha" style="display: inline-block; width: auto; vertical-align: top;">
        <label for="node-input-verify_captcha" style="width: 70%;">Verify Captcha</label>
    </div>
    <div class="form-row form-row-http-in-must_log_in">
        <label>&nbsp;</label>
        <input type="checkbox" id="node-input-must_log_in" style="display: inline-block; width: auto; vertical-align: top;">
        <label for="node-input-must_log_in" style="width: 70%;">Account must be logged in</label>
    </div>
    <div class="form-row form-row-http-in-must_be_active_account">
        <label>&nbsp;</label>
        <input type="checkbox" id="node-input-must_be_active_account" style="display: inline-block; width: auto; vertical-align: top;">
        <label for="node-input-must_be_active_account" style="width: 70%;">Account be active and logged in</label>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
        <label for="node-input-api_url"><i class="fa fa-globe"></i> API URL /api/[XXX]</label>
        <input id="node-input-api_url" type="text" placeholder="/url">
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
        <label for="node-input-api_output"><i class="fa fa-globe"></i> EXAMPLE SUCCESS JSON API OUTPUT</label>
        <textarea id="node-input-api_output" placeholder="{}"></textarea>
    </div>
...
<script type="text/x-red" data-template-name="http response">
    ...
    <div class="form-row">
        <label for="node-input-output_name">Output Name</label>
        <input type="text" id="node-input-output_name">
    </div>
    <div class="form-row">
        <label for="node-input-output_template_name">Output Template</label>
        <input type="text" id="node-input-output_template_name">
    </div>
    <div class="form-row">
        <label for="node-input-output_template_layout_name">Output Template Layout</label>
        <input type="text" id="node-input-output_template_layout_name">
    </div>
    <div class="form-row">
...
(function() {
    RED.nodes.registerType('http in',{
        ...
        defaults: {
            ...
            layout: {value: "",required:false},
            upload: {value:false},
            must_log_in: {value: false},
            must_be_active_account: {value: false},
    	    verify_captcha: {value: false},
            security_enabled: {value: false},
            api_enabled: {value: false},
            api_category: {value: ''},
    	    description: {value: ''},
	        security_type: {value: ""},
        ...
        oneditprepare: function() {
			...
			$(".form-row-http-in-security").show();		    
                } else {
            ...
            $.get( "get-layouts", function( data ) {
                $('#node-input-layout').html(`<option value="">None</option>`)
                for (var i = 0; i < data.length; i++) {
                    $('#node-input-layout').append(`<option value="${data[i].id}">${data[i].name}</option>`)
                }
                $('#node-input-layout').val(node.layout)
            });
            $.get( "get-kb-categories", function( data ) {
                $('#node-input-api_category').html(`<option value="">None</option>`)
                for (var i = 0; i < data.length; i++) {
                    $('#node-input-api_category').append(`<option value="${data[i].id}">${data[i].name}</option>`)
                }
                $('#node-input-api_category').val(node.api_category)
            });

            $.get( "get-knowledgebasis", function( data ) {
                $('#node-input-description').html(`<option value="">None</option>`)
                for (var i = 0; i < data.length; i++) {
                    $('#node-input-description').append(`<option value="${data[i].id}">${data[i].name}</option>`)
                }
            ...
            $.getJSON('getModels', function(models) {
                node.models = models;
                for (var n in node.models) {
                    $('#node-input-model').append('<option value="'+node.models[n]+'">'+node.models[n]+'</option>');
                }
            ...
            $('#node-input-model').change(function() {
                ...
                $('#node-input-required_fields').html('');
                $('#node-input-available_fields').html('');

                $.getJSON('getModelFields/' + (model || node.model), function(fields) {
                    node.fields = fields;
                    for (var n in node.fields) {
                        $('#node-input-required_fields').append('<option value="'+node.fields[n]+'">'+node.fields[n]+'</option>');
                        $('#node-input-available_fields').append('<option value="'+node.fields[n]+'">'+node.fields[n]+'</option>');
                    }
                    $('#node-input-required_fields').val(node.required_fields);
                    $('#node-input-available_fields').val(node.available_fields);
                });
            })
    ...
    RED.nodes.registerType('http response',{
        ...
        defaults: {
            ...
            headers: {value:{}},
            output_name: {value:"payload"},
            output_template_name: {value:""},
            output_template_layout_name: {value:""},
        },
        inputs:1,
        outputs:0,
        align: "right",
